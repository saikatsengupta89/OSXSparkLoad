...
//        }
       ...
       if (refresh_budget == 1) {
          ...
          val exist_dir = "hadoop fs -test -d ".concat(s"$hdfs_seg_cust_agg/time_key=$time_key/last_refresh_flag=Y/version_id=$version_id/amount_class=MGTBGT").!
           
          if (exist_dir == 0) {
              println (process_sumx_agg.time+" "+  "OSX CUST AGG - REMOVE AMOUNT CLASS - MGTBGT BEFORE BUDGET REBASE")
              val remove_dir = "hadoop fs -rm -r ".concat(s"$hdfs_seg_cust_agg/time_key=$time_key/last_refresh_flag=Y/version_id=$version_id/amount_class=MGTBGT").!
              if(remove_dir==0) {
               
                println (process_sumx_agg.time+" "+  "OSX CUST AGG - MGTBGT DIRECTORY FOR "+time_key+" IS REMOVED")
                //pw.println ("OSX BASE - EXISTING DIRECTORY FOR "+time_key+" IS DELETED BEFORE BASE AGG RELOAD. LOAD IN PROGRESS")
              }
           }
          
           println(process_sumx_agg.time+" "+"OSX CUST AGG - CREATING DUMMY TEMP VIEW WITH LATEST VERSION ID FOR BUDGET REFRESH")
           ...
           val cust_seg_data= sqlContext.sql (
              ...
              "GROUP BY AMOUNT_CLASS, CAST(LKP.VERSION_ID AS INT), BANKING_TYPE, BANKING_TYPE_CUSTOMER, BS_PL_FLAG, CURRENCY_CODE, CUSTOMER_NUMBER, "+
          ...
          println (process_sumx_agg.time+" "+  "OSX CUST AGG - WRITING BUDGET REBASE TO HDFS USING SNAPPPY COMPRESSION IN PARQUET FORMAT. WRITE PATH :"+ hdfs_seg_cust_agg)
          ...
          println(process_sumx_agg.time+" "+ "OSX CUST AGG - BUDGET REBASE COMPLETED")
           
       }
       
       else if (overwrite_flag==0 && ("hadoop fs -test -d ".concat(hdfs_seg_cust_agg_r).!)==0)
       {
