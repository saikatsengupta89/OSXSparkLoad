...
object load_customer_agg {
    ...
    def osx_seg_customer_agg_load (sqlContext:SQLContext, 
                                   time_key:String,
                                   sumx_data:DataFrame, 
                                   dom:DataFrame, 
                                   acc:DataFrame, 
       ...
       pw.println(today()+" OSX CUST - AGGREGATE LOADING INITIATED")
       
       val hdfs_seg_cust_agg_r= hdfs_seg_cust_agg+"/time_key="+time_key
       ...
       //DIRECTORY EXISTS ALREADY. REMOVE THE EXISTING DIRECTORY AND THEN APPEND DATA FOR THE TIMEKEY
        if(("hadoop fs -test -d ".concat(hdfs_seg_cust_agg_r).!)==0)  
        {
           ...
           print(remove_dir)
           if(remove_dir==0) {
             pw.println(" OSX CUST - EXISTING DIRECTORY FOR "+time_key+" IS DELETED BEFORE CUSTOMER AGG RELOAD. LOAD IN PROGRESS")
           }
        }
       
       pw.println(today()+" OSX CUST - DIMENSION TABLES INITIATED FOR LOOKUP")
       
       sumx_data.registerTempTable("OSX_DEAL_DATA")
       dom.registerTempTable("OSX_DOM")
       acc.registerTempTable("OSX_ACC_FPA")
       
       pw.println(today()+" OSX CUST - CUSTOMER AGGREGATE DATAFRAME CREATED")
       ...
       val cust_seg_data= sqlContext.sql (
          ...
          //pw.println(today()+"Customer aggregate loaded. Total Count" + cust_seg_data.count())
          
          pw.println(today()+ " OSX CUST - WRITING DATAFRAME TO HDFS USING SNAPPPY COMPRESSION IN PARQUET FORMAT. WRITE PATH :", hdfs_seg_cust_agg)
          ...
          */
          ...
          pw.println(today()+ " OSX CUST - AGGREGATE LOAD COMPLETED")
